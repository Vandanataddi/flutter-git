...
class DCR_Entry_new extends StatefulWidget {
  @override
  _MyHomePageState createState() => _MyHomePageState();
}

class _MyHomePageState extends State<DCR_Entry_new> {
  ...
  Future<Null> getData() async {
    ...
    try {
      ...
      String url = "/GetSavedMTPData_ForCalendar?RepId=${Constants_data.app_user["Rep_Id"]}&monthYear=$monthYear";
      ...
      for (int i = 0; i < mainData.length; i++) {
        if (Constants_data.stringToDate(mainData[i]["date"].toString(), "dd-MM-yyyy") == Constants_data.stringToDate(date.toString(), "dd-MM-yyyy")) {
    ...
    if (currentCalenderData == null) {
      return Column(
        children: [
          ...
          Expanded(
            child: Center(
              child: Text("MPT data is not available for this month"),
            ),
          )
        ],
      );
  ...
  Widget getRouteRow() {
    print("Current Route : ${currentCalenderData["route_desc"]}");
    print("Current Date : ${currentCalenderData["date"]}");
...
//                  ),
  ...
  Widget getSampleProductColumns() {
    ...
    for (int i = 0; i < sampleData.length; i++) {
      ...
      // if(!["",null].contains(sampleData[i]["qty"])){
      //   cnt_qty.text = sampleData[i]["qty"].toString();
      // }
      cnt_qty.text = sampleData[i]["qty"] == null || sampleData[i]["qty"] == "" ? "0" : sampleData[i]["qty"].toString();
      cnt_qty.selection = TextSelection.fromPosition(
          TextPosition(offset: cnt_qty.text.length));
      listCols.add(Stack(
        clipBehavior: Clip.none, children: <Widget>[
          Card(
            color: AppColors.light_blue_card_background,
                                ...
                                child: TextField(
                                  ...
                                  onChanged: (val) {
                                    ...
                                    int value = val == ""
                                        ? 0
                                        : int.parse(val);
                                    cnt_qty.text = value.toString();
                                    cnt_qty.selection = TextSelection.fromPosition(
                                        TextPosition(offset: cnt_qty.text.length));
                                    sampleData[i]["qty"] = value;
                                    // }
  ...
  Widget getPromotionalItemsColumns() {
    ...
    for (int i = 0; i < promotionalItemsData.length; i++) {
      TextEditingController cnt_qty = new TextEditingController();
      cnt_qty.text = promotionalItemsData[i]["qty"] == null || promotionalItemsData[i]["qty"] == "" ? "0" : promotionalItemsData[i]["qty"].toString();
      cnt_qty.selection = TextSelection.fromPosition(
          TextPosition(offset: cnt_qty.text.length));
      listCols.add(Stack(
        clipBehavior: Clip.none, children: <Widget>[
                            ...
                            child: Container(
                                ...
                                child: TextField(
                                  ...
                                  onChanged: (val) {
                                    ...
                                    cnt_qty.selection = TextSelection.fromPosition(
                                        TextPosition(offset: cnt_qty.text.length));
                                    promotionalItemsData[i]["qty"] = value;
                                  },
                                  style: new TextStyle(
                                    color: AppColors.black_color,
                                  ),
  ...
  saveData() async {
    ...
    if (strSelectedWorkType == "Field Work") {
      ...
      Map<String, dynamic> ObjectProfile = {
        ...
        "SessionID": "",
        "UserID": "${Constants_data.app_user["Rep_Id"]}",
        "HostName": "000000",
        "HostIP": "000000",
        "ViewSerialNo": 0,
        "MethodName": "",
      ...
      DCRDetail["rout_actual"] = currentCalenderData["route_code"];
      DCRDetail["rout_planned"] = currentCalenderData["route_code"];
      DCRDetail["work_type"] = currentCalenderData["work_type_code"];
        ...
        "DCRDetail": [
          {
            ...
            "visit_time": "$time",
            "Visit_type": "Doctor",
            "work_type": "${currentCalenderData["work_type_code"]}",
            "rout_actual": "",
            "rout_planned": "",
            "sr_no": "",
            "items": [],
